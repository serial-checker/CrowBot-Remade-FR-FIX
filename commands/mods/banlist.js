		...
		if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm) {
			message.guild.fetchBans()
				...
				.then(async bans => {
					...
					if (bList.length < 1) return message.channel.send(new Discord.MessageEmbed().setColor(color).setTimestamp().setFooter(`${client.config.name}`).setTitle("Aucun ban en cours"))
